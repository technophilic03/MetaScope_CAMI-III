...
# Convert MetaScope BLAST-reassigned tables into CAMI III taxonomic profiles.
#
# The arithmetic matches code/metascope_to_cami.R. CAMI defines PERCENTAGE as
# genome (cell) abundance, not read fraction, so every taxon is divided by its
# genome length and then renormalised.
#   https://cami-challenge.org/file-formats/#taxonomic-profiling
#
# The input is *.metascope_blast_reassigned.csv, written by blast_reassignment().
# It differs from a metascope_id table in three ways.
#
# 1. metascope_blast() writes some rows more than once. Its NCBI branch joins
#    the metascope_id table to a taxonomy table on TaxonomyID. A taxid holding k
#    rows on the left holds k rows on the right, so the join emits k^2. The
#    copies are identical in every column, so one of each is kept here. Summing
#    them instead would multiply that taxon by k.
# 2. An unknown taxid reads "NA" in a metascope_id table but 0 here, because
#    metascope_blast() replaces every NA with 0 before writing.
# 3. The blast_validated column marks the taxa whose reads BLAST agreed with.
#
# Usage:
#   Rscript code/metascope_b_to_cami.R \
#     --taxdump DIR --genome-lengths TSV --out FILE [--out-validated FILE] \
#     [--sample-map TSV] in1.csv [in2.csv ...]
#
...
#   --out             profile from every row; one @SampleID block per input file
#   --out-validated   second profile from the blast_validated rows only
#   --sample-map      optional headerless TSV, input basename <TAB> sample ID
#
# Read the --out-validated profile with care. metascope_blast() BLASTs only the
# top num_results taxa, so every taxon below that rank scores 0 and is
# unvalidated for want of evidence, not for failing a test. The filter is
# therefore an abundance cutoff at that rank, and it is not a BLAST decision.
#
# Without --sample-map the sample ID is the input basename with
# ".metascope_blast_reassigned.csv" removed. The CAMI III toy gold standard
# names its samples "0" to "19" while MetaScope writes "sample_0...", and OPAL
# pairs a prediction to a gold standard by sample ID, so the map is needed there.
#
# Rows whose taxid is unknown leave the profile and the rest are rescaled to
# 100. The run reports how much read fraction that removed.
...
percent_digits <- 6L
tol <- 1e-9
input_suffix <- "\\.metascope_blast_reassigned\\.csv$"
...
parse_args <- function(argv) {
  ...
  if (length(opts$inputs) == 0) {
    stop("No input .metascope_blast_reassigned.csv given", call. = FALSE)
  }
...
sample_ids_for <- function(inputs, map_path) {
  ids <- str_remove(basename(inputs), input_suffix)
  if (is.na(map_path)) {
    return(ids)
  }
  map_tbl <- read_tsv(
...
# Read one reassigned table and drop the copies metascope_id_species duplicated.
# Every column is read as text so that a row counts as a copy only when it
# matches another in all of them.
read_reassigned <- function(path, sample_id) {
  raw <- read_csv(
    path,
    col_types = cols(.default = col_character()),
    progress = FALSE
  )
  wanted <- c("TaxonomyID", "Genome", "EMProportion", "blast_validated")
  ...
  kept <- distinct(raw)
  em_all <- as.numeric(raw$EMProportion)
  em <- as.numeric(kept$EMProportion)
  if (anyNA(em)) {
    stop(sample_id, ": EMProportion is not a number in every row", call. = FALSE)
  }
  validated <- as.logical(kept$blast_validated)
  if (anyNA(validated)) {
    stop(
      sample_id,
      ": blast_validated is not TRUE or FALSE in every row",
      call. = FALSE
    )
  }
  message(sprintf(
    "%s: %d rows read, %d duplicate copies dropped (%.5f%% of reads), %d kept",
    sample_id,
    nrow(raw),
    nrow(raw) - nrow(kept),
    100 * (sum(em_all) - sum(em)),
    nrow(kept)
  ))
  # The copies are exact, so dropping them must leave one row per genome.
  repeated <- count(kept, TaxonomyID, Genome) |> filter(n > 1)
  if (nrow(repeated) > 0) {
    stop(
      sample_id,
      ": one genome still holds two rows after dropping copies: ",
      str_c(repeated$Genome, collapse = ", "),
      call. = FALSE
    )
  }
  tibble(taxid = kept$TaxonomyID, em = em, validated = validated)
}
...
profile_for_sample <- function(
  ...
  if (nrow(ms) == 0) {
    stop(sample_id, " (", label, "): no rows to profile", call. = FALSE)
  }

  moved <- merged$new[match(ms$taxid, merged$old)]
  taxid <- if_else(is.na(moved), ms$taxid, moved)

  # metascope_blast() writes 0 where accessionTaxa.sql had no taxid for an
  ...
  unnamed <- is.na(taxid) | taxid %in% c("NA", "", "0")
  stale <- !unnamed & is.na(match(taxid, nodes$taxid))
  dropped <- unnamed | stale
  dropped_pct <- 100 * sum(ms$em[dropped])
  if (any(stale)) {
    message(sprintf(
      "%s (%s): dropping %d taxid(s) absent from this taxdump: %s",
      sample_id,
      label,
      sum(stale),
  ...
  taxid <- taxid[!dropped]
  em_kept <- ms$em[!dropped]
  if (length(taxid) == 0) {
    stop(sample_id, " (", label, "): every row was dropped", call. = FALSE)
  }
  ...
  if (length(no_length) > 0) {
    stop(
      ...
      "): ",
      length(no_length),
      " taxid(s) absent from the genome length table: ",
      str_c(head(unique(no_length), 20), collapse = ", "),
      call. = FALSE
    )
  }

  # A merged taxid can collapse two rows onto one node.
...
check_profile <- function(p) {
  ...
  message(sprintf(
    paste(
      "%s (%s): %d input rows, %d merged taxids, %d rows dropped (%.4f%% of",
      "reads), %d taxa, %d species rows, species total %.6f%%"
    ),
    p$sample_id,
    p$label,
    p$n_in,
...
write_profile <- function(profiles, out_path) {
  walk(profiles, check_profile)
  dir.create(dirname(out_path), recursive = TRUE, showWarnings = FALSE)
  writeLines(flatten_chr(map(profiles, format_block)), out_path)
  message("Wrote ", out_path, " with ", length(profiles), " sample block(s)")
}
...
main <- function() {
  ...
  sample_ids <- sample_ids_for(opts$inputs, opts$sample_map)
  tables <- map2(opts$inputs, sample_ids, read_reassigned)

  write_profile(
    pmap(
      list(tables, sample_ids),
      profile_for_sample,
      label = "all rows",
      lengths_tbl = lengths_tbl,
      nodes = nodes,
  ...
  if (!is.na(opts$out_validated)) {
    validated <- map(tables, ~ filter(.x, validated))
    empty <- sample_ids[map_int(validated, nrow) == 0]
    if (length(empty) > 0) {
      stop(
        "No blast_validated row in sample(s): ",
        str_c(empty, collapse = ", "),
        call. = FALSE
      )
    }
    write_profile(
      pmap(
        list(validated, sample_ids),
        profile_for_sample,
        label = "validated",
        lengths_tbl = lengths_tbl,
        nodes = nodes,
